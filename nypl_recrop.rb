...
if matching_captures && matching_captures.length > 0
  ...
  crop_params = `./template_match_multiscale.py --template #{ARGV[0]}.jpg --image /Users/ying/documents/uw/Junior-quarter4/lab/tif/#{ARGV[0]}.tif`.chomp
  ...
  x1 = stereo_metadata['x1'] * x_scale # x1 on cropped tif correlating to low res
  x2 = stereo_metadata['x2'] * x_scale
  y1 = stereo_metadata['y1'] * y_scale
  y2 = stereo_metadata['y2'] * y_scale

  # store the metadata
  File.open("/Users/ying/documents/uw/Junior-quarter4/lab/meta/meta_#{ARGV[0]}.json","w") do |f|
  ...
  `convert #{ARGV[0]}_cropped.tif -crop #{cropped_width}x#{cropped_height}+#{x1}+#{y1} +repage /Users/ying/documents/uw/Junior-quarter4/lab/cropped/#{ARGV[0]}/imL.png`

  `convert #{ARGV[0]}_cropped.tif -crop #{cropped_width}x#{cropped_height}+#{x2}+#{y2} +repage /Users/ying/documents/uw/Junior-quarter4/lab/cropped/#{ARGV[0]}/imR.png`
