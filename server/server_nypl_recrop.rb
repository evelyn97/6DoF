...
if matching_captures && matching_captures.length > 0
  ...
  `wget -nc -O /projects/grail/6DOF/dataset_NYPL/#{ARGV[0]}.jpg '#{lowres_url}'`
  `wget -nc -O /projects/grail/6DOF/dataset_NYPL/tif/#{ARGV[0]}.tif '#{highres_url}'`

  # calculate the crop for the original image using multiscale template matching
  # $stdout.puts "Calculating crop..."
  crop_params = `./template_match_multiscale.py --template /projects/grail/6DOF/dataset_NYPL/#{ARGV[0]}.jpg --image /projects/grail/6DOF/dataset_NYPL/tif/#{ARGV[0]}.tif`.chomp

  # apply the crop
  `convert /projects/grail/6DOF/dataset_NYPL/tif/#{ARGV[0]}.tif -crop #{crop_params} +repage /projects/grail/6DOF/dataset_NYPL/#{ARGV[0]}_cropped.tif`

  # calculate dimensions
  lowres_dims = Dimensions.dimensions("/projects/grail/6DOF/dataset_NYPL/#{ARGV[0]}.jpg")
  highres_dims = Dimensions.dimensions("/projects/grail/6DOF/dataset_NYPL/#{ARGV[0]}_cropped.tif")
  ...
  # store the metadata
  File.open("/projects/grail/6DOF/dataset_NYPL/meta/meta_#{ARGV[0]}.txt", 'w+') {|f| f.write(stereo_metadata) }
  ...
  `convert /projects/grail/6DOF/dataset_NYPL/#{ARGV[0]}_cropped.tif -crop #{cropped_width}x#{cropped_height}+#{x1}+#{y1} +repage /projects/grail/6DOF/dataset_NYPL/cropped/#{ARGV[0]}/imL.png`

  `convert /projects/grail/6DOF/dataset_NYPL/#{ARGV[0]}_cropped.tif -crop #{cropped_width}x#{cropped_height}+#{x2}+#{y2} +repage /projects/grail/6DOF/dataset_NYPL/cropped/#{ARGV[0]}/imR.png`
